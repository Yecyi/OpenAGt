...
export function It2SetupPrompt(t0) {
    ...
    function renderInitialPrompt() {
      ...
      return (
        <Box flexDirection="column" gap={1}>
          <Text>
            To use native iTerm2 split panes for teammates, you need the <Text bold={true}>it2</Text> CLI tool.
          </Text>
          <Text dimColor={true}>This enables teammates to appear as split panes within your current window.</Text>
          <Box marginTop={1}>
            <Select
              options={options}
              onChange={(value) => {
  ...
  if ($[29] !== exitState || $[30] !== step) {
    t15 = step !== "installing" && step !== "verifying" && step !== "success" && (
  ...
  if (
    ...
    t16 = (
      <T0 flexDirection={t9} gap={t10} paddingBottom={t11}>
        {t12}
        {t13}
        {t15}
      </T0>
