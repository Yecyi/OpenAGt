...
export function HooksConfigMenu(t0) {
  ...
  const { hooksByEvent, totalHooksCount } = t20
  if (hooksDisabled_1) {
    let t21
    ...
    if ($[42] !== t22 || $[43] !== t23 || $[44] !== t24 || $[45] !== t25) {
      t26 = (
        <Text>
          All hooks are currently {t21}
          {t22}. You have {t23} configured {t24} that {t25} not running.
        </Text>
      )
      $[42] = t22
      $[43] = t23
    ...
    if ($[47] === Symbol.for("react.memo_cache_sentinel")) {
      t27 = (
        <Box marginTop={1}>
          <Text dimColor={true}>When hooks are disabled:</Text>
        </Box>
      )
    ...
    if ($[53] !== disabledByPolicy) {
      t32 = !disabledByPolicy && (
        <Text dimColor={true}>To re-enable hooks, remove "disableAllHooks" from settings.json or ask Claude.</Text>
      )
      $[53] = disabledByPolicy
      $[54] = t32
    } else {
    ...
    if ($[58] !== handleExit || $[59] !== t33) {
      t34 = (
        <Dialog title="Hook Configuration - Disabled" onCancel={handleExit} inputGuide={_temp6}>
          {t33}
        </Dialog>
      )
      $[58] = handleExit
