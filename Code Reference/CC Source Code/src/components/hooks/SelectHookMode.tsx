...
export function SelectHookMode(t0) {
  const $ = _c(19)
  const { selectedEvent, selectedMatcher, hooksForSelectedMatcher, hookEventMetadata, onSelect, onCancel } = t0
  const title =
  ...
  if (hooksForSelectedMatcher.length === 0) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
      t1 = (
        <Box flexDirection="column" gap={1}>
          ...
          <Text dimColor={true}>To add hooks, edit settings.json directly or ask Claude.</Text>
        </Box>
      )
      $[0] = t1
    } else {
    ...
    if ($[1] !== hookEventMetadata.description || $[2] !== onCancel || $[3] !== title) {
      t2 = (
        <Dialog title={title} subtitle={hookEventMetadata.description} onCancel={onCancel} inputGuide={_temp}>
          {t1}
        </Dialog>
      )
  ...
  if ($[10] !== onCancel || $[11] !== t2 || $[12] !== t3) {
    t4 = (
      <Box flexDirection="column">
        <Select options={t2} onChange={t3} onCancel={onCancel} />
      </Box>
    )
    $[10] = onCancel
    $[11] = t2
  ...
  if ($[14] !== hookEventMetadata.description || $[15] !== onCancel || $[16] !== t4 || $[17] !== title) {
    t5 = (
      <Dialog title={title} subtitle={t1} onCancel={onCancel}>
        {t4}
      </Dialog>
    )
