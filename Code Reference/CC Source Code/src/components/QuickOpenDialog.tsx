...
export function QuickOpenDialog(t0) {
  ...
  if ($[19] !== maxPathWidth) {
    t12 = (p_6, isFocused) => (
      <Text color={isFocused ? "suggestion" : undefined}>{truncatePathMiddle(p_6, maxPathWidth)}</Text>
    )
    $[19] = maxPathWidth
    $[20] = t12
  } else {
  ...
  if ($[21] !== preview || $[22] !== previewWidth || $[23] !== query) {
    t13 = (p_7) =>
      preview ? (
        ...
        <LoadingState message={"Loading preview\u2026"} dimColor={true} />
      )
    $[21] = preview
    $[22] = previewWidth
    $[23] = query
