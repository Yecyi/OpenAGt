...
function TeamDetailView(t0) {
  ...
  if ($[0] !== selectedIndex || $[1] !== teammates) {
    t2 =
      teammates.length === 0 ? (
        <Text dimColor={true}>No teammates</Text>
      ) : (
  ...
  if ($[3] !== onCancel || $[4] !== subtitle || $[5] !== t1 || $[6] !== t2) {
    t3 = (
      <Dialog title={t1} subtitle={subtitle} onCancel={onCancel} color="background" hideInputGuide={true}>
        {t2}
      </Dialog>
    )
    $[3] = onCancel
...
function TeammateListItem(t0) {
  ...
  if (
    ...
    t8 = (
      <Text color={t2} dimColor={shouldDim}>
        {t3}
        {t4}
        {t5}
        {t6}@{teammate.name}
    ...
    t10 = teammate.prompt && (
      <Box flexDirection="column">
        ...
        <Text>
          ...
          {stringWidth(teammate.prompt) > 80 && !promptExpanded && <Text dimColor={true}> (p to expand)</Text>}
        </Text>
      </Box>
    )
    $[25] = promptExpanded
