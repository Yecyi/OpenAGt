...
export function BackgroundTaskStatus(t0) {
  const $ = _c(48)
  const { tasksSelected, isViewingTeammate, teammateFooterIndex: t1, isLeaderIdle: t2, onOpenDialog } = t0
  ...
  if (allTeammates || (!showSpinnerTree && isViewingTeammate)) {
    ...
    const { startIndex, endIndex, showLeftArrow, showRightArrow } = t10
    let t11
        ...
        return (
          <React.Fragment key={pill_1.name}>
            ...
            <AgentPill
              ...
              onClick={() =>
                pill_1.taskId ? enterTeammateView(pill_1.taskId, setAppState) : exitTeammateView(setAppState)
              }
            />
          </React.Fragment>
        )
...
function AgentPill(t0) {
  const $ = _c(19)
  const { name, color, isSelected, isViewed, isIdle, onClick } = t0
...
function SummaryPill(t0) {
  const $ = _c(8)
  const { selected, onClick, children } = t0
  const [hover, setHover] = useState(false)
  const t1 = selected || hover
  let t2
  if ($[0] !== children || $[1] !== t1) {
    t2 = (
      <Text color="background" inverse={t1}>
        {children}
      </Text>
    )
    $[0] = children
  ...
  if ($[5] !== label || $[6] !== onClick) {
    t5 = (
      <Box onClick={onClick} onMouseEnter={t3} onMouseLeave={t4}>
        {label}
      </Box>
    )
    $[5] = label
