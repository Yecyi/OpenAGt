...
export function DreamDetailDialog(t0) {
  ...
  if (
    ...
    t7 =
      shown.length === 0 ? (
        <Text dimColor={true}>{task.status === "running" ? "Starting\u2026" : "(no text output)"}</Text>
      ) : (
        <>
          {hidden > 0 && (
            <Text dimColor={true}>
  ...
  if (
    ...
    t18 = (
      <T1 title={t8} subtitle={t9} onCancel={t10} color={t11} inputGuide={t12}>
        {t17}
      </T1>
    )
    $[55] = T1
  ...
  if ($[63] !== T2 || $[64] !== t13 || $[65] !== t14 || $[66] !== t15 || $[67] !== t16 || $[68] !== t18) {
    t19 = (
      <T2 flexDirection={t13} tabIndex={t14} autoFocus={t15} onKeyDown={t16}>
        {t18}
      </T2>
    )
    $[63] = T2
