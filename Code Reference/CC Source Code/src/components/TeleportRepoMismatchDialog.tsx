      ...
      availablePaths.length > 0 ? (
        <>
          <Box flexDirection="column" gap={1}>
            ...
            <Text>
              Open Claude Code in <Text bold={true}>{targetRepo}</Text>:
            </Text>
          </Box>
          {validating ? (
            <Box>
        ...
        <Box flexDirection="column" gap={1}>
          {errorMessage && <Text color="error">{errorMessage}</Text>}
          <Text dimColor={true}>Run claude --teleport from a checkout of {targetRepo}</Text>
        </Box>
      )
    $[8] = availablePaths.length
    $[9] = errorMessage
...
function _temp(path) {
  return {
    label: (
      <Text>
        Use <Text bold={true}>{getDisplayPath(path)}</Text>
      </Text>
    ),
    value: path,
  }
