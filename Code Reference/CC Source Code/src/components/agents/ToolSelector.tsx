...
export function ToolSelector(t0) {
  const $ = _c(69)
  const { tools, initialTools, onComplete, onCancel } = t0
  let t1
  if ($[0] !== tools) {
  ...
  if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = (toolName) => {
      ...
      setSelectedTools((current) =>
        current.includes(toolName) ? current.filter((t_1) => t_1 !== toolName) : [...current, toolName],
      )
    }
    $[14] = t6
  } else {
  ...
  if ($[64] !== handleKeyDown || $[65] !== t16 || $[66] !== t19 || $[67] !== t21) {
    t22 = (
      <Box flexDirection="column" marginTop={1} tabIndex={0} autoFocus={true} onKeyDown={handleKeyDown}>
        {t16}
        {t17}
        {t19}
        {t21}
