...
export function MCPListPanel(t0) {
  ...
  if ($[32] !== selectableItems) {
    t14 = (agentServer_0) =>
      selectableItems.findIndex((item_1) => item_1.type === "agent-server" && item_1.agentServer === agentServer_0)
    $[32] = selectableItems
    $[33] = t14
  } else {
    t14 = $[33]
  ...
  if ($[54] !== agentServers || $[55] !== renderAgentServerItem) {
    t24 = agentServers.length > 0 && (
      <Box flexDirection="column" marginBottom={1}>
        <Box paddingLeft={2}>
          <Text bold={true}>Agent MCPs</Text>
        </Box>
        {[...new Set(agentServers.flatMap(_temp6))].map((agentName) => (
  ...
  if ($[71] !== handleCancel || $[72] !== t21 || $[73] !== t29) {
    t30 = (
      <Dialog title="Manage MCP servers" subtitle={t21} onCancel={handleCancel} hideInputGuide={true}>
        {t29}
      </Dialog>
    )
    $[71] = handleCancel
