...
export function LogSelector(t0) {
  ...
  bb0: {
    ...
    if ($[39] !== baseFilteredLogs || $[40] !== searchQuery) {
      ...
      t23 = baseFilteredLogs.filter((log_5) => {
        ...
        return (
          displayedTitle.includes(query) || branch_0.includes(query) || tag.includes(query) || prInfo.includes(query)
        )
      })
      $[39] = baseFilteredLogs
      $[40] = searchQuery
  ...
  if ($[187] !== agenticSearchState.results || $[188] !== agenticSearchState.status) {
    t66 = agenticSearchState.status === "results" && agenticSearchState.results.length > 0 && (
  ...
  if ($[194] !== agenticSearchState.status || $[195] !== filteredLogs) {
    t68 = agenticSearchState.status === "error" && filteredLogs.length === 0 && (
      <Box paddingLeft={1} marginBottom={1} flexShrink={0}>
        <Text dimColor={true} italic={true}>
          No matching sessions found.
        </Text>
    ...
    t70 =
      agenticSearchState.status === "searching" ? null : viewMode === "rename" && focusedLog ? (
        ...
        <TreeSelect
          ...
          onExpand={(nodeId_0) => {
            const sessionId_3 =
              typeof nodeId_0 === "string" && nodeId_0.startsWith("group:") ? nodeId_0.substring(6) : null
            if (sessionId_3) {
          ...
          onCollapse={(nodeId_1) => {
            const sessionId_4 =
              typeof nodeId_1 === "string" && nodeId_1.startsWith("group:") ? nodeId_1.substring(6) : null
            if (sessionId_4) {
    ...
    t71 = (
      <Box paddingLeft={2}>
        {exitState.pending ? (
          ...
          <Text dimColor={true}>
            <Byline>
              <Text>{isSearching && false ? "Searching\u2026" : "Type to Search"}</Text>
