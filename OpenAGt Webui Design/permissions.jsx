      ...
      <div className="pc">
        ...
        <div className="pc-body">
          <aside className="pc-queue">
            <h3 className="pc-h">
              Pending <span className="ct">{queue.length}</span>
            </h3>
            ...
            <h3 className="pc-h" style={{ marginTop: 24 }}>
              Audit · Recent
            </h3>
            {history.map((h) => (
          ...
          <section className="pc-detail">
            {sel ? (
              <PermissionDetail p={sel} resolved={resolved[sel.id]} onDecide={decide} />
            ) : (
    ...
    <div className="pcd">
      ...
      <div className="pcd-grid">
        ...
        <div className="pcd-block">
          ...
          <p className="pcd-p">
            {p.retryable
              ? "Yes — agent may retry under a softer rule."
              : "No — this requires explicit operator decision."}
          </p>
        </div>
        ...
        <div className="pcd-block pcd-wide">
          ...
          <div className="pcd-bound">
            <div>
              ...
              <span className="v">{p.boundary.sandbox}</span>
            </div>
            <div>
              <span className="k">Filesystem</span>
              <span className="v">{p.boundary.filesystem}</span>
            </div>
            <div>
              <span className="k">Network</span>
              <span className="v">{p.boundary.network}</span>
            </div>
          </div>
        </div>
        ...
        <div className="pcd-block pcd-wide">
          ...
          <p className="pcd-p">
            <span className="mono">{p.requestedBy}</span> on session <span className="mono">{p.sessionId}</span> at{" "}
      ...
      <div className="pcd-acts">
        {resolved ? (
          <span className={`pcd-resolved ${resolved}`}>
            {resolved === "allow" ? "✓ Allowed once" : resolved === "always" ? "✓ Always allowed" : "✗ Blocked"}
          </span>
        ) : (
          <>
