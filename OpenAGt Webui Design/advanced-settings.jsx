...
const INITIAL_NODES = [
  // Column 1 — Input
  { id: "n_input",     kind: "input",    x:   60, y: 120, title: "User input",       sub: "message · files · paste",          knobs: { roots: ["~/code/openagt"], svgAsText: true, pasteSummary: true } },
  // Column 2 — Routing
  { id: "n_router",    kind: "router",   x:  340, y: 100, title: "Router",            sub: "auto · effort-aware",              knobs: { effort: "medium", model: "claude-sonnet-4.5", small: "claude-haiku-4-5" } },
  // Column 3 — Primary agent
  { id: "n_primary",   kind: "agent",    x:  640, y:  60, title: "primary",           sub: "coordinator · sonnet-4.5",         knobs: { steps: 28, temp: 1.0, color: "#0d0d0d" } },
  // Column 4 — Tools fan-out
  { id: "n_read",      kind: "tool",     x:  960, y: -40, title: "read()",            sub: "low risk",                          knobs: { enabled: true, scope: "all" } },
  { id: "n_edit",      kind: "tool",     x:  960, y:  60, title: "edit()",            sub: "med risk",                          knobs: { enabled: true, scope: "all" } },
  { id: "n_bash",      kind: "tool",     x:  960, y: 160, title: "bash()",            sub: "high · sandboxed",                  knobs: { enabled: true, scope: "all" } },
  { id: "n_task",      kind: "tool",     x:  960, y: 260, title: "task()",            sub: "spawn subagents",                   knobs: { enabled: true, scope: "primary-only" } },
  // Column 5 — Sandbox
  { id: "n_sandbox",   kind: "sandbox",  x: 1280, y: 140, title: "Sandbox",           sub: "seatbelt · enforced",              knobs: { backend: "auto", network: "deny", brokerTtl: 300 } },
  // Column 6 — Subagents (called via task)
  { id: "n_research",  kind: "agent",    x: 1280, y: 320, title: "researcher",        sub: "subagent · read-only",             knobs: { steps: 18, temp: 0.7, color: "#3b5d6a" } },
  { id: "n_writer",    kind: "agent",    x: 1280, y: 440, title: "code-writer",       sub: "subagent · scoped writes",         knobs: { steps: 22, temp: 0.4, color: "#3a5a3a" } },
  // Column 7 — Feedback gate
  { id: "n_perm",      kind: "feedback", x: 1600, y: 220, title: "Permission gate",   sub: "ask · correctable",                knobs: { defaultAction: "ask", continueOnDeny: false, allowCorrection: true } },
  // Column 8 — Output
  { id: "n_output",    kind: "output",   x: 1900, y: 240, title: "Output stream",     sub: "reply · events · patches",         knobs: { showReasoning: true, expandTools: true, eventReplay: true } },
];
...
function AdvancedSettings({ open, onClose }) {
  ...
  const onBoardMouseDown = (e) => {
    ...
    if (e.target === boardRef.current || e.target.classList.contains("adv2-bg") || e.target.classList.contains("adv2-grid")) {
      setSelectedId(null);
  ...
  const onBoardDragOver = (e) => { e.preventDefault(); e.dataTransfer.dropEffect = "copy"; };
  const onBoardDrop = (e) => {
    e.preventDefault();
    ...
    setNodes(ns => [...ns, {
      id, kind, x: p.x - NODE_W/2, y: p.y - NODE_H_BASE/2,
  ...
  const updateNode = (id, patch) => {
    setNodes(ns => ns.map(n => n.id === id ? { ...n, ...patch, knobs: { ...n.knobs, ...(patch.knobs || {}) } } : n));
    setDirty(true);
  };
  const deleteNode = (id) => {
  ...
  const reset = () => {
    ...
    setSelectedId("n_primary"); setDirty(false);
  ...
  return (
    <div className="adv2-scrim" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
        ...
        <header className="adv2-bar">
          ...
          <div className="adv2-bar-tools">
            <div className="adv2-toolgrp">
              <button className={`adv2-tool ${tool === "select" ? "on" : ""}`} onClick={() => setTool("select")} title="Select (V)">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M3 2l9 5-4 1-1 4-4-10z"/></svg>
              </button>
              <button className={`adv2-tool ${tool === "pan" ? "on" : ""}`} onClick={() => setTool("pan")} title="Pan (H)">
              ...
              <button className={`adv2-tool ${tool === "connect" ? "on" : ""}`} onClick={() => setTool("connect")} title="Connect (C)">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4">
                  <circle cx="3.5" cy="8" r="1.5"/><circle cx="12.5" cy="8" r="1.5"/><line x1="5" y1="8" x2="11" y2="8"/>
                </svg>
              </button>
            </div>
            <div className="adv2-toolgrp">
              <button className="adv2-tool" onClick={() => setView(v => ({ ...v, k: Math.min(2.2, v.k * 1.15) }))} title="Zoom in">+</button>
              <button className="adv2-tool" onClick={fit} title="Fit board">{Math.round(view.k * 100)}%</button>
              <button className="adv2-tool" onClick={() => setView(v => ({ ...v, k: Math.max(0.25, v.k * 0.87) }))} title="Zoom out">−</button>
            </div>
            <div className="adv2-toolgrp">
              <button className={`adv2-tool ${showGrid ? "on" : ""}`} onClick={() => setShowGrid(g => !g)} title="Toggle grid">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1"><path d="M2 2h12v12H2zM6 2v12M10 2v12M2 6h12M2 10h12"/></svg>
              </button>
              <button className={`adv2-tool ${snap ? "on" : ""}`} onClick={() => setSnap(s => !s)} title="Snap to grid">snap</button>
            </div>
          </div>
          <div className="adv2-bar-right">
            {dirty && <span className="adv2-dirty"><span className="d"></span>Unsaved</span>}
            <button className="adv2-mini ghost" onClick={reset}>Reset workflow</button>
            <button className="adv2-mini" onClick={() => { setDirty(false); }} disabled={!dirty}>Apply</button>
        ...
        <div className="adv2-body">
          ...
          <aside className="adv2-stencil">
            ...
            <div className="adv2-stencil-list">
              {STENCIL.map(s => {
                ...
                return (
                  ...
                  >
                    <span className="adv2-sten-glyph" style={{ borderColor: meta.accent, color: meta.accent }}>{meta.glyph}</span>
            ...
            <div className="adv2-rail-foot">
              ...
              <div className="adv2-legend-row"><span className="lw lw-data"></span>data flow</div>
              <div className="adv2-legend-row"><span className="lw lw-route"></span>route decision</div>
              <div className="adv2-legend-row"><span className="lw lw-tool"></span>tool call</div>
              <div className="adv2-legend-row"><span className="lw lw-spawn"></span>spawn subagent</div>
              <div className="adv2-legend-row"><span className="lw lw-gate"></span>permission gate</div>
              <div className="adv2-legend-row"><span className="lw lw-return"></span>return value</div>
            </div>
          </aside>
          ...
          >
            ...
            <div
              className="adv2-canvas"
              style={{ transform: `translate(${view.x}px, ${view.y}px) scale(${view.k})` }}
            >
              {/* Edges */}
              <svg className="adv2-wires" width="3000" height="2000" viewBox="0 0 3000 2000">
                <defs>
                  <marker id="adv2-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
                ...
                {edges.map(e => {
                  ...
                  if (!a || !b) return null;
                  const x1 = a.x + NODE_W, y1 = a.y + 36;
                  const x2 = b.x,         y2 = b.y + 36;
                  const cx1 = x1 + Math.max(60, (x2 - x1) / 2);
                  const cx2 = x2 - Math.max(60, (x2 - x1) / 2);
                  const d = `M ${x1} ${y1} C ${cx1} ${y1}, ${cx2} ${y2}, ${x2} ${y2}`;
                  return (
                    <g key={e.id} className={`adv2-wire wire-${e.kind}`} onClick={(ev) => { ev.stopPropagation(); deleteEdge(e.id); }}>
                      ...
                      {e.label && (
                        <g transform={`translate(${(x1+x2)/2}, ${(y1+y2)/2 - 8})`}>
                          ...
                          <text className="adv2-wire-lab" textAnchor="middle" y="3">{e.label}</text>
                        </g>
                      )}
                    </g>
                  );
                ...
                {linking && (() => {
                  ...
                  if (!a) return null;
                  const x1 = a.x + NODE_W, y1 = a.y + 36;
                  const x2 = linking.x, y2 = linking.y;
                  const cx1 = x1 + Math.max(60, (x2 - x1) / 2);
                  const cx2 = x2 - Math.max(60, (x2 - x1) / 2);
                  return <path d={`M ${x1} ${y1} C ${cx1} ${y1}, ${cx2} ${y2}, ${x2} ${y2}`} className="adv2-wire-pending" />;
                })()}
              </svg>
            ...
            <div className="adv2-canvas-foot">
              ...
              <span><b>{edges.length}</b> wires</span>
          ...
          <aside className="adv2-inspector">
            {selected ? (
              <Inspector node={selected} onChange={(p) => updateNode(selected.id, p)} onDelete={() => deleteNode(selected.id)} edges={edges} nodes={nodes} />
            ) : (
              <EmptyInspector />
            )}
          </aside>
  ...
  return (
    ...
    >
      ...
      <div className="adv2-node-head">
        <span className="adv2-node-glyph" style={{ borderColor: meta.accent, color: meta.accent }}>{meta.glyph}</span>
      ...
      <div
        className="adv2-port adv2-port-in"
  ...
  return (
    <div className="adv2-insp adv2-insp-empty">
      ...
      <p className="adv2-insp-empty-lede">
        Click a node on the canvas to edit its configuration. Drag from the stencil rail
        to add new stages. Drag from a node's right edge to wire it to another node.
      </p>
      <div className="adv2-insp-empty-tips">
        <div><kbd>V</kbd><span>Select / move</span></div>
        <div><kbd>H</kbd><span>Pan canvas</span></div>
        <div><kbd>⌫</kbd><span>Delete selected</span></div>
        <div><kbd>⌘</kbd><kbd>scroll</kbd><span>Zoom</span></div>
      </div>
    </div>
  );
}
    ...
    <div className="adv2-insp">
      <div className="adv2-insp-head">
        <div>
          ...
          <input
            className="adv2-insp-title"
            value={node.title}
            onChange={(e) => onChange({ title: e.target.value })}
          />
        </div>
        <button className="adv2-mini ghost" onClick={onDelete}>Delete</button>
      </div>
      <input
        ...
        <div className="adv2-wires-pane">
          <div className="adv2-wires-col">
            ...
            {incoming.map(e => {
              ...
              return <div key={e.id} className="adv2-wires-row"><span className={`pip pip-${e.kind}`}></span>{from?.title}</div>;
            })}
          </div>
          <div className="adv2-wires-col">
            ...
            {outgoing.map(e => {
              ...
              return <div key={e.id} className="adv2-wires-row"><span className={`pip pip-${e.kind}`}></span>{to?.title}</div>;
            })}
          </div>
        </div>
      </section>
  ...
  return (
    <div className="adv2-segs">
      {options.map(o => {
        ...
        return <button key={v} className={`adv2-seg ${value === v ? "on" : ""}`} onClick={() => onChange(v)}>{lab}</button>;
      })}
    </div>
  );
}
    ...
    <section className="adv2-insp-sec">
      ...
      <K label="Roots" hint="Folders the agent may read.">
        <div className="adv2-chips">
          {(k.roots || []).map((r, i) => (
            <span key={i} className="adv2-chip">{r}<button onClick={() => onChange({ knobs: { roots: k.roots.filter((_, j) => j !== i) } })}>×</button></span>
          ))}
          <button className="adv2-chip add" onClick={() => onChange({ knobs: { roots: [...(k.roots || []), "~/code/new-path"] } })}>+ path</button>
        </div>
      </K>
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Reasoning effort" hint="Drives the variant chosen for downstream agents.">
        <Segs value={k.effort} onChange={(v) => onChange({ knobs: { effort: v } })} options={["low","medium","high","deep"]} />
      </K>
      <K label="Default model">
        <Sel value={k.model} onChange={(v) => onChange({ knobs: { model: v } })} options={[
          "claude-opus-4","claude-sonnet-4.5","claude-haiku-4-5","gpt-5.1","gpt-4o","gemini-2.5-pro"
        ]} />
      </K>
      <K label="Small model" hint="Used for cheap utility steps.">
        <Sel value={k.small} onChange={(v) => onChange({ knobs: { small: v } })} options={[
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Step budget" hint="Hard cap on iterations of this agent's loop.">
        <Num value={k.steps || 16} onChange={(v) => onChange({ knobs: { steps: v } })} min={1} max={120} step={1} suffix="steps" />
      </K>
      <K label="Temperature">
      ...
      <K label="Color" hint="Shown in transcript and DAG.">
        <div className="adv2-swatches">
          {["#0d0d0d","#3b5d6a","#3a5a3a","#7a3b2e","#6a4a1a","#5a3a5a"].map(c => (
            <button key={c} className={`adv2-sw ${k.color === c ? "on" : ""}`} style={{ background: c }} onClick={() => onChange({ knobs: { color: c } })} />
          ))}
        </div>
      </K>
    </section>
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Scope" hint="Which agents may invoke this tool.">
        <Segs value={k.scope || "all"} onChange={(v) => onChange({ knobs: { scope: v } })} options={[
          { value: "all", label: "All" },
...
function SandboxKnobs({ node, onChange }) {
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Backend">
        <Sel value={k.backend || "auto"} onChange={(v) => onChange({ knobs: { backend: v } })} options={[
          "auto","process","seatbelt","windows_native","landlock"
        ]} />
      </K>
      <K label="Network policy">
        <Segs value={k.network || "deny"} onChange={(v) => onChange({ knobs: { network: v } })} options={[
      ...
      <K label="Broker TTL" hint="How long an approved command stays approved.">
        <Num value={k.brokerTtl ?? 300} onChange={(v) => onChange({ knobs: { brokerTtl: v } })} min={0} max={3600} step={30} suffix="sec" />
      </K>
    </section>
  );
}
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Default action">
        <Segs value={k.defaultAction || "ask"} onChange={(v) => onChange({ knobs: { defaultAction: v } })} options={["ask","allow","always","deny"]} />
      </K>
...
function OutputKnobs({ node, onChange }) {
  ...
  return (
    <section className="adv2-insp-sec">
      ...
      <K label="Show reasoning"><Toggle on={k.showReasoning !== false} onChange={(v) => onChange({ knobs: { showReasoning: v } })} /></K>
      <K label="Expand tool parts"><Toggle on={k.expandTools !== false} onChange={(v) => onChange({ knobs: { expandTools: v } })} /></K>
      <K label="Event replay"><Toggle on={k.eventReplay !== false} onChange={(v) => onChange({ knobs: { eventReplay: v } })} /></K>
    </section>
  );
}
...
function MiniMap({ nodes, edges, view, setView, boardRef }) {
  ...
  const xmin = Math.min(...xs, 0) - 80, xmax = Math.max(...xs, 0) + NODE_W + 80;
  const ymin = Math.min(...ys, 0) - 80, ymax = Math.max(...ys, 0) + NODE_H_BASE + 80;
  const span = Math.max(xmax - xmin, 1), spanY = Math.max(ymax - ymin, 1);
  const sx = W / span, sy = H / spanY;
  const r = boardRef.current?.getBoundingClientRect();
