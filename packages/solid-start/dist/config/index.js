...
const runtimeImportPath = (url) => fileURLToPath(url).replaceAll("\\", "/");
export function solidStart(options) {
    ...
    const appEntryPath = globSync(join(root, start.appRoot, "app.{j,t}sx"))[0]?.replaceAll("\\", "/");
    if (!appEntryPath) {
            ...
            callers: [
                {
                    ...
                    getRuntimeCode: () => `import { createServerReference } from "${runtimeImportPath(new URL("../server/server-runtime", import.meta.url))}"`,
                    replacer: opts => `createServerReference('${opts.functionId}')`
                },
                {
                    envConsumer: "server",
                    envName: VITE_ENVIRONMENTS.server,
                    getRuntimeCode: () => `import { createServerReference } from '${runtimeImportPath(new URL("../server/server-fns-runtime", import.meta.url))}'`,
