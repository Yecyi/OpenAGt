var __rewriteRelativeImportExtension = (this && this.__rewriteRelativeImportExtension) || function (path, preserveJsx) {
    if (typeof path === "string" && /^\.\.?\//.test(path)) {
        return path.replace(/\.(tsx)$|((?:\.d)?)((?:\.[^./]+?)?)\.([cm]?)ts$/i, function (m, tsx, d, ext, cm) {
            return tsx ? preserveJsx ? ".jsx" : ".js" : d && (!ext || !cm) ? m : (d + ext + "." + cm.toLowerCase() + "js");
        });
    }
    return path;
};
import { createStore } from "solid-js/store";
import { createComponent, getHydrationKey, getOwner, hydrate } from "solid-js/web";
/**
 *
 * Read more: https://docs.solidjs.com/solid-start/reference/client/mount
 */
export function mount(fn, el) {
    if (import.meta.env.START_ISLANDS) {
        const map = new WeakMap();
        async function mountIsland(el) {
            if (el.dataset.css) {
                let css = JSON.parse(el.dataset.css);
                for (let href of css) {
                    if (!document.querySelector(`link[href="${href}"]`)) {
                        let link = document.createElement("link");
                        link.rel = "stylesheet";
                        link.href = href;
                        document.head.appendChild(link);
                    }
                }
            }
            let mod = await import(__rewriteRelativeImportExtension(
            /* @vite-ignore */
            import.meta.env.MANIFEST["client"].chunks[el.dataset.id.split("#")[0]].output
                .path, true));
            if (!mod || !el.dataset.hk)
                return;
            let Component = mod[el.dataset.id.split("#")[1]];
            let hk = el.dataset.hk;
            // _$DEBUG("hydrating island", el.dataset.island, hk.slice(0, hk.length - 1) + `1-`, el);
            let props = createStore({
                ...JSON.parse(el.dataset.props),
                get children() {
                    const p = el.getElementsByTagName("solid-children");
                    getHydrationKey();
                    [...p].forEach(a => {
                        a.__$owner = getOwner();
                    });
                    return;
                }
            });
            map.set(el, props);
            hydrate(() => createComponent(Component, props[0]), el, {
                renderId: hk.slice(0, hk.length - 1) + `${1 + Number(el.dataset.offset)}-`,
                owner: lookupOwner(el)
            });
            delete el.dataset.hk;
            el.dataset.hkk = hk;
        }
        let queue = [];
        let queued = false;
        function runTaskQueue(info) {
            while (info.timeRemaining() > 0 && queue.length) {
                mountIsland(queue.shift());
            }
            if (queue.length) {
                requestIdleCallback(runTaskQueue);
            }
            else
                queued = false;
        }
        const hydrateIslands = () => {
            const islands = document.querySelectorAll("solid-island[data-hk]");
            const assets = new Set();
            islands.forEach((el) => el.dataset.id && assets.add(el.dataset.id));
            Promise.all([...assets].map(asset => import(
            /* @vite-ignore */ import.meta.env.MANIFEST["client"].chunks[asset.split("#")[0]].output.path)))
                .then(() => {
                islands.forEach((el) => {
                    if (el.dataset.when === "idle" && "requestIdleCallback" in window) {
                        if (!queued) {
                            queued = true;
                            requestIdleCallback(runTaskQueue);
                        }
                        queue.push(el);
                    }
                    else
                        mountIsland(el);
                });
            })
                .catch(e => console.error(e));
        };
        function lookupOwner(el) {
            const parent = el.closest("solid-children");
            return parent && parent.__$owner;
        }
        hydrateIslands();
        return;
    }
    return hydrate(fn, el);
}
