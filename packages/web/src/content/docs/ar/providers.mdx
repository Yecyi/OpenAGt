---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenAGt ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenAGt.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenAGt.
...
OpenAGt Zen هي قائمة نماذج يوفّرها فريق OpenAGt وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenAGt. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenAGt Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/auth).
...
يعمل مثل أي مزوّد آخر في OpenAGt واستخدامه اختياري بالكامل.

---

## OpenAGt Go

OpenAGt Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenAGt، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenAGt.

1. شغّل الأمر `/connect` في TUI، واختر `OpenAGt Go`، ثم انتقل إلى [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
لاستخدام Amazon Bedrock مع OpenAGt:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenAGt غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم OpenAGt نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenAGt
...
لاستخدام Google Vertex AI مع OpenAGt:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة OpenAGt تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenAGt مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenAGt. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenAGt:
...
3. انسخ مفتاح API لاستخدامه في OpenAGt.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenAGt، يجب سحب معلومات النموذج محليا:
...
OpenAGt Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenAGt. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenAGt Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenAGt.
...
تسمح حقول `limit` لـ OpenAGt بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
