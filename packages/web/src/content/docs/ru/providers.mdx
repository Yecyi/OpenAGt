...
OpenAGt Zen — это список моделей, предоставленный командой opencode, которые были
протестировано и проверено на хорошую работу с opencode. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с OpenAGt Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `OpenAGt Zen` и перейдите по адресу [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
OpenAGt Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `OpenAGt Go` и перейдите по адресу [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `OpenAGt`, Области применения: `api`
...
OpenAGt Zen — это список протестированных и проверенных моделей, предоставленный командой opencode. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>OpenAGt Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **OpenAGt Zen**.
