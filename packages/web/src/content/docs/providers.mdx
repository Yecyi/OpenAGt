---
title: Providers
description: Using any LLM provider in OpenAGt.
---

import config from "../../../config.mjs"
export const console = config.console

OpenAGt uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenAGt config.
...
You can customize the providers through the `provider` section in your OpenAGt
config.
...
OpenAGt Zen is a list of models provided by the OpenAGt team that have been
tested and verified to work well with OpenAGt. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenAGt Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenAGt Zen`, and head to [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
It works like any other provider in OpenAGt and is completely optional to use.

---

## OpenAGt Go

OpenAGt Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenAGt team that have been
tested and verified to work well with OpenAGt.

1. Run the `/connect` command in the TUI, select `OpenAGt Go`, and head to [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
To use Amazon Bedrock with OpenAGt:
...
OpenAGt. Anthropic explicitly prohibits this.

Previous versions of OpenAGt came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in OpenAGt with zero setup:
...
GitLab Duo support in OpenAGt is experimental. Features, configuration, and
behavior may change in future releases.
:::

OpenAGt integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
OpenAGt uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenAGt
...
When a `duo-workflow-*` model is selected, OpenAGt will:
...
4. Route tool execution requests through OpenAGt's permission-gated tool system
...
To use Google Vertex AI with OpenAGt:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenAGt conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenAGt conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for OpenAGt. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with OpenAGt:
...
3. Copy the API key for use in OpenAGt.
...
6. **Important**: Before using cloud models in OpenAGt, you must pull the model information locally:
...
OpenAGt Zen is a list of tested and verified models provided by the OpenAGt team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenAGt Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenAGt API key.
...
The `limit` fields allow OpenAGt to understand how much context you have left. Standard providers pull these from models.dev automatically.
