---
title: ACP Support
description: Use OpenAGt in any ACP-compatible editor.
---

OpenAGt supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use OpenAGt via ACP, configure your editor to run the `opencode acp` command.

The command starts OpenAGt as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'OpenAGt' agent in the AI Chat agent selector.
...
To use OpenAGt as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use OpenAGt as the ACP agent for chat.
...
OpenAGt works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your OpenAGt config
