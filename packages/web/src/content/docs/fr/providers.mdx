---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAGt utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenAGt.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenAGt
configuration.
...
OpenAGt Zen est une liste de modèles fournis par l'équipe OpenAGt qui ont été
testé et vérifié pour fonctionner correctement avec OpenAGt. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenAGt Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenAGt et son utilisation est totalement facultative.

---

## OpenAGt Go

OpenAGt Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe OpenAGt qui ont été
testé et vérifié pour fonctionner correctement avec OpenAGt.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenAGt Go` et rendez-vous sur [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
Pour utiliser Amazon Bedrock avec OpenAGt :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenAGt n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
OpenAGt utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenAGt
...
Pour utiliser Google Vertex AI avec OpenAGt :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation OpenAGt en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenAGt répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour OpenAGt. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenAGt :
...
3. Copiez la clé API à utiliser dans OpenAGt.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenAGt, vous devez extraire les informations du modèle localement :
...
OpenAGt Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenAGt. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenAGt Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenAGt API.
...
Les champs `limit` permettent à OpenAGt de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
