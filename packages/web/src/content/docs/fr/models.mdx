...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez OpenAGt.
...
Voici plusieurs modèles qui fonctionnent bien avec OpenAGt, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration OpenAGt.
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [OpenAGt Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. OpenAGt est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

OpenAGt est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque OpenAGt démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration OpenAGt.
