...
Det er to typer agenter i OpenAGt; primære agenter og underagenter.
...
OpenAGt kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenAGt kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenAGt kommer med to innebygde primære agenter og to innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker OpenAGt modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenAGt-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenAGt-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenAGt Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/Yecyi/OpenAGt).
:::
