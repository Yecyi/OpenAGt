---
title: Konfiguracija
description: Korištenje OpenAGt JSON konfiguracije.
---

Možete konfigurirati OpenAGt koristeći JSON konfiguracijski fajl.

---

## Format

OpenAGt podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu OpenAGt konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se OpenAGt pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`github.com/Yecyi/OpenAGt/config.json`**](https://github.com/Yecyi/OpenAGt/raw/dev/packages/web/dist/config.json).

TUI konfiguracija koristi [**`github.com/Yecyi/OpenAGt/tui.json`**](https://github.com/Yecyi/OpenAGt/raw/dev/packages/web/dist/tui.json).
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenAGt server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenAGt konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenAGt pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenAGt će se vratiti na `"build"` s upozorenjem.
...
OpenAGt će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenAGt **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenAGt sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite OpenAGt da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
