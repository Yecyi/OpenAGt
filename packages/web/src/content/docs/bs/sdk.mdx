...
Mo啪ete zatra啪iti strukturirani JSON izlaz od modela specificiranjem `format` sa JSON 拧emom. Model 膰e koristiti `StructuredOutput` alat da vrati validirani JSON koji odgovara va拧oj 拧emi.
...
| `json_schema` | Vra膰a validirani JSON koji odgovara pru啪enoj 拧emi                   |
...
| Polje        | Tip             | Opis                                                        |
| ------------ | --------------- | ----------------------------------------------------------- |
| `type`       | `'json_schema'` | Obavezno. Odre膽uje JSON schema na膷in rada                   |
| `schema`     | `object`        | Obavezno. JSON Schema objekt koji definira strukturu izlaza |
| `retryCount` | `number`        | Opcionalno. Broj ponovnih poku拧aja validacije (default: 2)  |

### Rukovanje gre拧kama

Ako model ne uspije proizvesti validan strukturirani izlaz nakon svih ponovnih poku拧aja, odgovor 膰e uklju膷ivati `StructuredOutputError`:
...
1. **Navedite jasne opise** u svojstvima va拧e 拧eme kako biste pomogli modelu da razumije koje podatke treba izdvojiti
2. **Koristite `required`** da odredite koja polja moraju biti prisutna
3. **Dr啪ite 拧eme fokusiranim** - slo啪ene ugnije啪膽ene 拧eme modelu mogu biti te啪e za ispravno popunjavanje
4. **Postavite odgovaraju膰i `retryCount`** - pove膰ajte za slo啪ene 拧eme, smanjite za jednostavne

---

## API-ji

SDK izla啪e sve server API-je kroz type-safe klijent.
