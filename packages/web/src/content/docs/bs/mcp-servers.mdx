...
Možete dodati vanjske alate u OpenAGt koristeći _Model Context Protocol_, ili MCP. OpenAGt podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenAGt Config](https://github.com/Yecyi/OpenAGt/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
OpenAGt automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenAGt će:
...
Ako server zahtijeva autentifikaciju, OpenAGt će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenAGt sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenAGt, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenAGt konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenAGt sa vašim Sentry nalogom.
