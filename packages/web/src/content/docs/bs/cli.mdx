---
title: CLI
description: OpenAGt CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAGt CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa OpenAGt.
...
Pokrenite OpenAGt terminalski korisnički interfejs.
...
OpenAGt CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte OpenAGt agentima.
...
Priključite terminal na već pokrenut OpenAGt backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim OpenAGt backend-om. Na primjer:
...
OpenAGt pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se OpenAGt pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u OpenAGt.
...
Pokrenite OpenAGt u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti OpenAGt server (npr. http://localhost:4096)    |
...
Pokrenite OpenAGt headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenAGt-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše OpenAGt sesije.
...
Uvezite podatke sesije iz JSON datoteke ili OpenAGt dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili OpenAGt dijeljenog URL-a.
...
Pokrenite OpenAGt headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenAGt-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte OpenAGt i uklonite sve povezane datoteke.
...
Ažurira OpenAGt na najnoviju verziju ili određenu verziju.
...
OpenAGt CLI prihvata sljedeće globalne zastavice.
...
OpenAGt se može konfigurirati pomoću varijabli okruženja.
