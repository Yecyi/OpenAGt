---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAGt koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenAGt konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenAGt
config.
...
OpenAGt Zen je lista modela koje je obezbedio OpenAGt tim koji su bili
testirano i potvrđeno da dobro radi sa OpenAGt. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenAGt Zen.
:::
...
Radi kao i svaki drugi provajder u OpenAGt i potpuno je opcionalan za korištenje.

---

## OpenAGt Go

OpenAGt Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenAGt tim i koji su testirani i verificirani da dobro rade s OpenAGt-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenAGt Go` i idite na [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
Da biste koristili Amazon Bedrock s OpenAGt:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenAGt.
:::
...
OpenAGt koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenAGt
...
Za korištenje Google Vertex AI s OpenAGt:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki OpenAGt razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenAGt razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za OpenAGt. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenAGt:
...
3. Kopirajte API ključ za korištenje u OpenAGt.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenAGt, morate lokalno povući informacije o modelu:
...
OpenAGt Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenAGt tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenAGt Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenAGt Zen**.

```txt
   /connect
```

3. Unesite svoj OpenAGt API ključ.
...
Polja `limit` omogućavaju OpenAGt da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite OpenAGt konfiguraciju i:
