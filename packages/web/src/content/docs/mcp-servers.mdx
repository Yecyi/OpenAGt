...
You can add external tools to OpenAGt using the _Model Context Protocol_, or MCP. OpenAGt supports both local and remote servers.
...
You can define MCP servers in your [OpenAGt Config](https://github.com/Yecyi/OpenAGt/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
OpenAGt automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenAGt will:
...
If the server requires authentication, OpenAGt will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenAGt will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in OpenAGt, alongside built-in tools. So you can manage them through the OpenAGt config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenAGt to your Sentry account.
