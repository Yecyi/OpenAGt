---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenAGt.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAGt fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenAGt avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenAGt, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenAGt. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenAGt. _Alias_: `/quit`, `/q`
