...
In OpenAGt esistono due tipi di agenti: agenti primari e subagenti.
...
OpenAGt include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenAGt include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenAGt include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, OpenAGt usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenAGt sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenAGt usa il formato `provider/model-id`. Per esempio, se stai usando [OpenAGt Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/Yecyi/OpenAGt).
:::
