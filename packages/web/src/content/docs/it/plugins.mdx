---
title: Plugin
description: Scrivi plugin per estendere OpenAGt.
---

I plugin ti permettono di estendere OpenAGt agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalit脿, integrare servizi esterni o modificare il comportamento predefinito di OpenAGt.
...
I plugin vengono caricati da tutte le sorgenti e tutti gli hook vengono eseguiti in sequenza. L'ordine di caricamento 猫:
...
Un plugin 猫 un **modulo JavaScript/TypeScript** che esporta una o pi霉 funzioni di plugin. Ogni funzione riceve un oggetto di contesto e restituisce un oggetto di hook.
...
OpenAGt esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di OpenAGt, pu貌 inviare automaticamente notifiche di sistema quando una risposta 猫 pronta o quando una sessione va in errore.
:::
...
L'helper `tool` crea uno strumento personalizzato che opencode pu貌 chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/Yecyi/OpenAGt/docs/sdk) per i dettagli.
...
Quando `output.prompt` 猫 impostato, sostituisce completamente il prompt di compaction predefinito. In questo caso l'array `output.context` viene ignorato.
