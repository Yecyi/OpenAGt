---
title: CLI
description: Opzioni e comandi della CLI di OpenAGt.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di OpenAGt, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con OpenAGt in modo programmatico.
...
Avvia l'interfaccia testuale di OpenAGt (interfaccia utente da terminale).
...
La CLI di OpenAGt include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per OpenAGt.
...
Collega un terminale a un backend server di OpenAGt già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend OpenAGt remoto. Per esempio:
...
OpenAGt si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando OpenAGt si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in OpenAGt.
...
Avvia un server OpenAGt headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Elenca tutte le sessioni OpenAGt.
...
Mostra statistiche di utilizzo token e costo per le sessioni OpenAGt.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione OpenAGt.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione OpenAGt.
...
Avvia un server OpenAGt headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a OpenAGt tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla OpenAGt e rimuove tutti i file correlati.
...
OpenAGt può essere configurato tramite variabili d'ambiente.
