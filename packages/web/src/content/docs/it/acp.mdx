---
title: Supporto ACP
description: Usa OpenAGt in qualunque editor compatibile con ACP.
---

OpenAGt supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare OpenAGt via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia OpenAGt come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'OpenAGt' nel selettore agenti di AI Chat.
...
Per usare OpenAGt come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare OpenAGt come agente ACP per la chat.
...
OpenAGt funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione OpenAGt
