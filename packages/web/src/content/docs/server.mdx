---
title: Server
description: Interact with OpenAGt server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openag serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an OpenAGt client can use.
...
```bash
openag serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `openag.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
openag serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openag`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `openag serve` and `openag web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password openag serve
```
...
When you run `openag` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the OpenAGt server to interact with OpenAGt programmatically.
:::

This architecture lets OpenAGt support multiple clients and allows you to interact with OpenAGt programmatically.

You can run `openag serve` to start a standalone server. If you have the
openag TUI running, `openag serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenAGt [IDE](/docs/ide) plugins.
...
The OpenAGt server exposes the following APIs.
