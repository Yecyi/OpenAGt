...
:::caution
Modi werden jetzt über die Option `agent` in der OpenAGt-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in OpenAGt können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die OpenAGt-Konfiguration.
...
OpenAGt verfügt über zwei integrierte Modi.
...
Wenn keine Temperatur angegeben ist, verwendet OpenAGt modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale OpenAGt-Konfiguration als auch die projektspezifische Konfiguration.
