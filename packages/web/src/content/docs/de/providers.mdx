---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAGt verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenAGt-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenAGt anpassen
config.
...
OpenAGt Zen ist eine Liste von Modellen, die vom OpenAGt-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenAGt funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenAGt Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenAGt aus und gehen Sie zu [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenAGt und ist völlig optional.

---

## OpenAGt Go

OpenAGt Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenAGt-Team getestet und verifiziert wurden, dass sie gut mit OpenAGt funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenAGt Go` und gehen Sie zu [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
So verwenden Sie Amazon Bedrock mit OpenAGt:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenAGt ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenAGt wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenAGt ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenAGt-Konfiguration hinzufügen.
...
um das Token im OpenAGt-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenAGt verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenAGt
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenAGt:
...
So verwenden Sie Google Vertex AI mit OpenAGt:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenAGt ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenAGt konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede OpenAGt-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenAGt-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenAGt für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OpenAGt über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OpenAGt über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenAGt konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit OpenAGt:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenAGt.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenAGt verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenAGt Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenAGt-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenAGt Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenAGt API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenAGt-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenAGt-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenAGt:
...
Sie können Modelle auch über Ihre OpenAGt-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenAGt verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann OpenAGt erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenAGt-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenAGt-Konfiguration übereinstimmt.
