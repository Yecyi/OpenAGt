...
Sie können externe Tools zu OpenAGt hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. OpenAGt unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [OpenAGt Config](https://github.com/Yecyi/OpenAGt/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
OpenAGt übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird OpenAGt Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von OpenAGt beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `opencode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert OpenAGt die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in OpenAGt verfügbar. Sie können sie also wie jedes andere Tool über die OpenAGt-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und OpenAGt mit Ihrem Sentry-Konto zu verbinden.
