---
title: Konfiguration
description: Ved at bruge OpenAGt JSON konfig.
---

Du kan konfigurere OpenAGt ved hjælp af en JSON-konfigurationsfil.

---

## Format

OpenAGt understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Placer din globale OpenAGt-konfiguration i `~/.config/opencode/opencode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Når OpenAGt starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Konfigurationsfilen har et skema, der defineres i [**`github.com/Yecyi/OpenAGt/config.json`**](https://github.com/Yecyi/OpenAGt/raw/dev/packages/web/dist/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din OpenAGt-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din OpenAGt-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger OpenAGt at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din OpenAGt-konfiguration, gennem indstillingen `theme`.
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OpenAGt falde tilbage til `"build"` med en advarsel.
...
OpenAGt vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide OpenAGt med brugerdefinerede værktøjer, kroge og integrationer.
...
Dette er nyttigt, når du vil begrænse OpenAGt til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
