---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAGt bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenAGt-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i OpenAGt-konfigurationen.
...
OpenAGt Zen er en liste over modeller leveret af OpenAGt-teamet, som er blevet testet og verificeret til at fungere godt med OpenAGt. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenAGt Zen.
:::
...
Det fungerer som alle andre udbydere i OpenAGt og er helt valgfrit at bruge.

---

## OpenAGt Go

OpenAGt Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenAGt-teamet, som er testet og verificeret til at fungere godt med OpenAGt.

1. Kør kommandoen `/connect` i TUI, vælg `OpenAGt Go`, og gå til [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
Sådan bruger du Amazon Bedrock med OpenAGt:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenAGt understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
OpenAGt bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenAGt
...
Sådan bruger du GitHub Copilot-abonnementet med OpenAGt:
...
Sådan bruger du Google Vertex AI med OpenAGt:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver OpenAGt-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenAGt-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for OpenAGt. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenAGt:
...
3. Kopier API-nøglen til brug i OpenAGt.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenAGt, skal du hente modelinformationen lokalt:
...
OpenAGt Zen er en liste over testede og verificerede modeller leveret af OpenAGt-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenAGt Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenAGt API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenAGt:
...
`limit`-felterne lader OpenAGt forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
