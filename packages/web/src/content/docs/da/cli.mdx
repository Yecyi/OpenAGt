---
title: CLI
description: OpenAGt CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAGt CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med OpenAGt programmatisk.
...
Start OpenAGt terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende OpenAGt backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern OpenAGt backend. For eksempel:
...
OpenAGt drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når OpenAGt starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i OpenAGt.
...
Start en hovedløs OpenAGt-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator OpenAGt sessionsholder.
...
Liste over alle OpenAGt sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine OpenAGt-sessioner.
...
Importer sessionsdata fra en JSON fil eller OpenAGt del URL.
...
Start en hovedløs OpenAGt-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenAGt via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller OpenAGt og fjern alle relaterede filer.
...
Opdaterer OpenAGt til den nyeste version eller en specifik version.
...
OpenAGt kan konfigureres ved hjælp af miljøvariabler.
