...
Du kan tilføje eksterne værktøjer til OpenAGt ved hjælp af _Model Context Protocol_ eller MCP. OpenAGt understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [OpenAGt Config](https://github.com/Yecyi/OpenAGt/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
OpenAGt håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil OpenAGt:
...
Hvis serveren kræver godkendelse, vil OpenAGt bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil OpenAGt opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem OpenAGt-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde OpenAGt til din Sentry-konto.
