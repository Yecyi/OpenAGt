...
Puede agregar herramientas externas a OpenAGt utilizando el _Model Context Protocol_ o MCP. OpenAGt admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OpenAGt Config](https://github.com/Yecyi/OpenAGt/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
OpenAGt maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OpenAGt:
...
Si el servidor requiere autenticación, OpenAGt le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OpenAGt almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OpenAGt, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OpenAGt como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OpenAGt a su cuenta Sentry.
