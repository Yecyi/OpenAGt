---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenAGt.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAGt usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenAGt.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenAGt
configuración.
...
OpenAGt Zen es una lista de modelos proporcionados por el equipo OpenAGt que han sido
probado y verificado para funcionar bien con OpenAGt. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenAGt Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/auth).
...
Funciona como cualquier otro proveedor en OpenAGt y su uso es completamente opcional.

---

## OpenAGt Go

OpenAGt Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de OpenAGt que han sido
probado y verificado para funcionar bien con OpenAGt.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenAGt Go` y diríjase a [github.com/Yecyi/OpenAGt/auth](https://github.com/Yecyi/OpenAGt/zen).
...
Para usar Amazon Bedrock con OpenAGt:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenAGt no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
OpenAGt utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenAGt
...
Para utilizar Google Vertex AI con OpenAGt:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación OpenAGt como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenAGt enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para OpenAGt. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenAGt:
...
3. Copie la clave API para usarla en OpenAGt.
...
6. **Importante**: Antes de usar modelos en la nube en OpenAGt, debe obtener la información del modelo localmente:
...
OpenAGt Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenAGt. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenAGt Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenAGt Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenAGt API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenAGt:
...
Los campos `limit` le permiten a OpenAGt comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
