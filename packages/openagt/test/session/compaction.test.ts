...
function fake(
  ...
  return {
    ...
    failToolCall: Effect.fn("TestSessionProcessor.failToolCall")(() => Effect.succeed(true)),
...
describe("session.compaction.isOverflow", () => {
  ...
  // When limit.input is set, overflow detection must still reserve room for
  // the next model response. Otherwise compaction can trigger only after the
  // input budget is already exhausted.
  ...
  it.live(
    "matches context-output headroom when limit.input is absent",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 200_000, output: 32_000 })
  ...
  it.live(
    "keeps input-limit and context-only models symmetric near output headroom",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        ...
        expect(yield* compact.isOverflow({ tokens, model: withInputLimit })).toBe(true)
        expect(yield* compact.isOverflow({ tokens, model: withoutInputLimit })).toBe(true)
      }),
    ),
  )
