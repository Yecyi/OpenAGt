...
- Author package entrypoint is `@openagt/plugin/tui`.
...
- Import types from `@openagt/plugin/tui`.
- `@openagt/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openagt/plugin/tui"
...
- `opencode plugin <module>` and TUI install both run install 鈫?manifest read 鈫?config patch.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@openagt/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openagt/plugin/tui`.
