...
import type { AssistantMessage, Part, Provider, ToolPart, UserMessage, TextPart, ReasoningPart } from "@openagt/sdk/v2"
...
export function Session() {
  ...
  const providers = createMemo(() => Model.indexProviders(sync.data.provider))
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}openagt -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  const model = createMemo(() =>
    Model.getProviderModelName(ctx.providers(), props.message.providerID, props.message.modelID),
  )
