    ...
    await Instance.provide({
      ...
      fn: async () => {
        const records = Flag.OPENCODE_PURE
          ? []
          : (config.plugin_origins ?? []).filter((item) => {
              const enabled = ConfigPlugin.pluginOptions(item.spec)?.enabled
              if (enabled === false) {
                log.info("skipping disabled tui plugin", {
                  path: ConfigPlugin.pluginSpecifier(item.spec),
                  source: item.source,
                })
                return false
              }
