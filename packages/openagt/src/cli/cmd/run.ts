...
import { fallback, printTaskPart, printToolFailed, printToolPart } from "./run-printers"

export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run openagt with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openagt server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENAGT_SERVER_PASSWORD, falls back to OPENCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
