...
├── logger.ts           # Logger - 结构化日志
├── runner.ts           # Runtime 辅助工具
└── index.ts           # 导出入口
```
...
用于 Layer 组合时的去重，防止重复初始化同一个 Service。
...
export class MemoMap {
  constructor()
  readonly get: <I, E>(key: Layer.Key<I, E>) => Layer.Layer<I, E> | undefined
  readonly set: <I, E>(key: Layer.Key<I, E>, layer: Layer.Layer<I, E>) => Layer.Layer<I, E>
  readonly memoize: <I, E>(layer: Layer.Layer<I, E>) => Layer.Layer<I, E>
}
```
...
Effect/Promise 桥接，用于在 Effect 上下文外部调用 Effect 程序。
...
启动时初始化全局 Runtime，确保 Effect 上下文在应用生命周期内可用。

```typescript
// src/effect/bootstrap-runtime.ts
// 在应用入口调用一次
Effect.runPromise(bootstrap())
```
...
// 结构化日志
export const Logger = Context.Tag<LoggerService>()
```
