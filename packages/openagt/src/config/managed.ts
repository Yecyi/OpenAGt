...
const LEGACY_MANAGED_PLIST_DOMAIN = "ai.opencode.managed"

// Keys injected by macOS/MDM into the managed plist that are not OpenAGt config
...
export async function readManagedPreferences() {
  ...
  const paths = [
    ...
    path.join("/Library/Managed Preferences", user, `${LEGACY_MANAGED_PLIST_DOMAIN}.plist`),
    path.join("/Library/Managed Preferences", `${LEGACY_MANAGED_PLIST_DOMAIN}.plist`),
  ]
