...
export const DANGEROUS_CMDS = [
  ...
  'Start-Process -Verb RunAs', 'certutil.exe', 'bitsadmin.exe',
  'mshta.exe', 'wscript.exe', 'cscript.exe', 'regsvr32.exe',
  'rundll32.exe', 'powershell -enc', 'powershell -EncodedCommand',
]
```
...
const KEEP_PATTERNS = [
  /^(file_)?read/i,
  /^[A-Z][a-z]+(?:Error|Exception|Warning)/,
  /diff --git/,
]
const TRUNCATE_PATTERNS = [
  /stdout|stderr|output|result/i,
]
```
...
export function makeRuntime<I, S, E>(service: Context.Service<I, S>, layer: Layer.Layer<I, E>) {
  ...
  return {
    ...
    runPromiseExit: <A, Err>(fn, options?) =>
...
export class CircuitBreaker {
  ...
  isOpen(): boolean { return this.failures >= this.threshold }
  canAttempt(): boolean {
...
function validatePattern(pattern: string) {
  if (pattern === "*")
    return { valid: false, message: "Bare '*' pattern is too permissive." }
...
Database.transaction((tx) => {
  const row = tx.select({ seq: EventSequenceTable.seq })
...
}, { behavior: "immediate" })  // IMMEDIATE 事务保证并发安全
```
...
export function replay(event: SerializedEvent, options?: { publish: boolean }) {
  const latest = Database.use((db) =>
    db.select({ seq: EventSequenceTable.seq })
      .from(EventSequenceTable)
  ...
  if (event.seq !== latest + 1)
    throw new Error(`Sequence mismatch: expected ${latest + 1}, got ${event.seq}`)
...
*报告生成时间：2026-04-21*
*分析版本：OpenAGt v1.14.17, Claude Code v2.1.88*
