...
> 目标：基于 OpenCode/OpenAG 当前源码，吸收 Claude Code reference 与 Hermes Agent 的有效设计，形成更准确、更可执行的技术判断。
...
1. OpenAG 最适合继续建立在 `packages/opencode` 之上，而不是迁移到 Hermes 的 Python 运行时或试图复刻 Claude Code 的闭源单体架构。
2. OpenAG 当前已经具备可演进成"高质量开源 agent helper"的关键骨架：
   - 多 provider 抽象
   - Effect 驱动的 session / tool / event 主链
   - HTTP + SSE/WebSocket 的 client/server 架构
   - 可扩展的工具注册与 MCP 接入
   - 独立 Flutter 客户端目录
3. 最近一轮实现后，OpenAG 已经不再只是"计划接入"：
...
**OpenAG 的正确方向不是做 Claude Code 的像素级复刻，也不是把 Hermes 全盘移植进来，而是以 OpenCode 为稳定底座，吸收 Claude Code 的产品工程 discipline，以及 Hermes 的长期记忆与运行策略。**
...
3. reference code、OpenAG 当前状态、未来规划三者混在一起，导致读者难以判断哪些是事实，哪些是建议。
...
OpenAG 当前仍然是一个以 `packages/opencode` 为核心的 TypeScript/Bun 项目，具备下面这些现实优势：
...
2. `packages/opencode/src` 已经具备较完整的模块化边界：
   ...
   - `permission`
   - `mcp`
3. 这意味着 OpenAG 不是从零搭壳，而是在一个已经具备可扩展运行时的项目上迭代。
...
**OpenAG 已经进入"能力成形、工程收口中"的状态，而不是"功能空白"或"稳定完成"。**
...
OpenAG 现在已经把这个分层思想接进来了，这是正确方向。后续重点不在"是否继续三层"，而在：
...
OpenAG 想做成真正可 daily use 的开源 agent helper，必须补这类细节，而不是只堆功能点。
...
OpenAG 不应该为了"像 Claude Code"而回退到大型单体文件。

#### B. 闭源特性开关依赖

reference 中可见大量 feature-gated 内部模块。对 OpenAG 来说，这一模式的参考价值有限，因为：
...
OpenAG 当前最接近的自然延伸不是马上接十几个平台，而是：
...
这对 OpenAG 的启发不是照着做一套 Python memory system，而是：
...
#### A. Python 运行时并不适合作为 OpenAG 新底座

Hermes 的运行时和 OpenAG 当前栈差异太大：
...
这些都很吸引人，但对 OpenAG 当前阶段来说，优先级必须排在以下事项之后：
...
## 8. OpenAG 与两份 reference 的架构对照

### 8.1 运行时模型

| 维度 | OpenAG / OpenCode | Claude Code reference | Hermes Agent |
|------|------------------|----------------------|--------------|
...
| 适合 OpenAG 的继承方式 | 直接作为底座继续演进 | 参考执行纪律与 UX | 参考长期运行与离线策略 |

### 8.2 Agent loop

| 维度 | 当前 OpenAG 状态 | 技术实现 | 评估 |
|------|-----------------|---------|------|
...
| 维度 | 当前 OpenAG 状态 | 技术实现 | 评估 |
|------|-----------------|---------|------|
| 多 provider | 强 | `ProviderID` branded type + `Info` config schema | 这是 OpenAG 相比 reference 的现实优势 |
...
## 9. OpenAG 当前最有价值的差异化优势

和两份 reference 相比，OpenAG 当前真正有潜力做成差异化产品的点，不是"更多功能"，而是下面三条。

### 9.1 开源、模块化、多 provider

Claude Code 很强，但闭源、Anthropic 中心化、很多能力对外不可复制。

Hermes 很完整，但技术栈与 OpenAG 不同。

OpenAG 如果做对了，会具备一个非常独特的位置：
...
这是 OpenAG 相比 CLI 单体 agent 最大的产品潜力。
...
虽然 OpenAG 没有走到 Claude Code 那种超大单体程度，但 `session/prompt.ts` 已经开始承担过多职责：
...
1. OpenAG 现在已经不是"概念阶段"，而是"核心能力已经成形，但工程收口还在进行中"的阶段。
