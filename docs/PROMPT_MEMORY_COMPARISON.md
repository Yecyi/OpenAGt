# Prompt 与 Memory 系统对比：OpenAGt、CC Source Code、Codex
...
3. **国际化**：大量英文指令硬编码；若 OpenAGt 多语言场景要对标，需结构化抽取文案。

### 5.3 Codex

1. **compact 过短**：在极复杂 rollout 上可能丢失 CC 九段式中的细节；可考虑 **动态展开**（token 预算内附加 checklist）。
2. **Memory 启动管道**：异步、lease、两阶段失败重试——运维与调试面复杂；OpenAGt 若借鉴需配套可观测性与失败降级（跳过 memory 仅 compact）。
