...
| Windows native sandbox readiness gate | implemented | Helper probe/status now exposes machine-readable readiness (`ready`, `setup_required`, `admin_verification_required`, `acl_apply_required`, `backend_unavailable`), ACL apply mode, helper path/hash, setup state, supported network policies, and admin-gate report metadata. Broker selection and the shell decision pipeline consume readiness so `failure_policy=closed` blocks native execution until readiness is `ready`; preflight-only admin reports are not accepted as admin verification. |
