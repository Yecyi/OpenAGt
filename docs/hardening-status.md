...
| Windows native sandbox helper substrate | partial | `openagt-sandbox-win` crate, setup/probe CLI wiring, packaged-helper discovery, release packaging hooks, Job Object timeout containment, helper-side path preflight, restricted-token launch path, canonical filesystem grant-plan validation, rollback-aware ACL transaction planning, DACL backup/apply/rollback APIs, and an exec-path ACL wrapper exist. ACL apply remains explicit-gated and the helper deliberately reports unavailable until principal binding, filesystem ACL enforcement, and WFP enforcement are available and verified. |
