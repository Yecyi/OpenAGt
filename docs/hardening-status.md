...
| Windows native sandbox readiness surface | implemented | Helper probe/status and `GET /global/sandbox/status` now expose machine-readable readiness (`ready`, `setup_required`, `admin_verification_required`, `acl_apply_required`, `backend_unavailable`), ACL apply mode, helper path/hash/protocol, setup state, supported network policies, next action, and admin-gate report metadata. Settings > Sandbox renders this status with refresh and does not present the process backend as OS-native isolation. Broker selection and the shell decision pipeline consume readiness so `failure_policy=closed` blocks native execution until readiness is `ready`; preflight-only admin reports are not accepted as admin verification. |
| Windows native sandbox helper substrate | partial | `openagt-sandbox-win` crate, setup/probe CLI wiring, packaged-helper discovery, release packaging hooks, Job Object timeout containment, helper-side path preflight, restricted-token launch path, restricted-code SID binding, canonical filesystem grant-plan validation, rollback-aware ACL transaction planning, DACL backup/apply/rollback APIs, an exec-path ACL wrapper, capability-driven filesystem probe, setup/elevation/enforcement diagnostics, gated ACL enforcement integration coverage, WFP setup lifecycle, security-descriptor scoped WFP user conditions, setup-gated `network_policy=none` capability reporting, an admin-gated loopback-block execution test, and table-driven WFP filter install/remove hooks exist. ACL apply remains explicit-gated, `network_policy=none` requires elevated setup plus manual/CI opt-in verification before it is treated as default-ready, and `network_policy=loopback` remains deferred. |
...
- Default-on Windows OS-native sandbox enforcement remains blocked until explicit-gated ACL apply is promoted and setup-gated WFP `network_policy=none` has stable admin-gate evidence; run `bun run verify:windows-sandbox-admin` from an elevated Windows terminal for the admin-only WFP execution gate. `network_policy=loopback` remains deferred.
