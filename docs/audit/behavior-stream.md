...
The 2024–2026 LLM-behavior research (CoT faithfulness 25–39% on misaligned hints; Anthropic's emotion-concepts paper §1.3 reward-hacking case B) shows the agent's stated reasoning is an unreliable audit signal. Internal state and external presentation are separable; what the agent _says_ it will do diverges from what it _actually does_ a meaningful fraction of the time.
