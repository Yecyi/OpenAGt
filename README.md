...
- **Prompt-affect lint.** [`bun run check:prompt-affect`](script/audit-prompt-affect.ts) gates the prompt corpus against three rule families:
  - **high-affect** — caps emphasis (e.g. `URGENT`, `CRITICAL`, `MUST NOT`) and threat framing (e.g. `violation`, `forbidden`, `supersedes any other instruction`) that primes desperate-vector activation per the Anthropic emotion-concepts paper.
  - **affect-instruction** — telling the model how to feel (e.g. `stay calm`, `you are confident`, `do not panic`); the same paper §1.5 shows suppression instructions teach masking, not removal — they generalize into deceptive behavior rather than fixing the underlying representation.
  - **anti-escape** — phrases that close the legitimate "stop and ask" affordance (e.g. `keep going until`, `MUST iterate`, `solve it autonomously`, `NEVER end your turn`); Wiser Human (2025) measured a ~32× swing in agentic-misalignment rates between prompts that allow vs close this channel.
